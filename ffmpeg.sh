                    ...
                    if [ -f \"\$video\" ]; then
                        ...
                        if [ \"\$HAS_AUDIO\" == \"audio\" ]; then
                            ffmpeg -re -i \"\$video\" \
                                ...
                                -filter_complex \"\
                                [0:v][1:v]overlay=shortest=1[v]; \
                                [v]eq=contrast=${CONTRAST}:brightness=${BRIGHTNESS}[vout]; \
                                [0:a][2:a]amix=inputs=2:duration=first:weights='1 0.0001'[a]; \
                                [a]volume=${VOLUME}[aout]\" \
                                -map \"[vout]\" -map \"[aout]\" \
